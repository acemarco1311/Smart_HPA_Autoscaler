...
class MicroserviceManager:
    ...
    def __init__(self, microservice_name, min_reps, max_reps,
        ...
        # update max_reps from disk
        self.sync_max_reps()
        # microservice scaled and failed to update disk
        # read from current replicas of ms deployment
        desired_reps = get_desired_reps(self.microservice_name)
        if desired_reps is not None:
            if self.get_current_arm_max_reps() is not None:
                if desired_reps > self._current_arm_max_reps:
                    self._current_arm_max_reps = desired_reps

        self.foo = 0 # just ensure fault injection will not happen in the first round
    ...
    def get_max_reps(self):
        ...
        if self.get_current_arm_max_reps() is None:
            return self.max_reps
        else:
            return self.get_current_arm_max_reps()
    ...
    def Execute(self, arm_decision):
        ...
        # failure injection
        # only scale 70% of the time
        number = random.randint(1, 10)
        # no injection in the first exchange round
        if number > 7 and self.foo != 0:
            return None

        self.foo += 1

        # update first, then scale, avoid dying while scaling without writing to disk
        ...
        # because scaling will not return None
        # it returns a string always
        # scaling only return None in MCA
        # when MM not available
        scaling_result = None
        while scaling_result is None:
    ...
    def sync_max_reps(self):
        with self.disk_lock:
            # load state if state disk exist
