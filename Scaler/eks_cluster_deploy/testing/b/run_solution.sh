...
# deploy MCA
kubectl apply -f solution_manifest/microservice-capacity-analyzer.yaml
...
target_ip=$2
locust -f locustfile.py --host=http://"$target_ip" --headless -u 300 -r 1 --run-time=600 2>&1 --csv="$locust_path" --csv-full-history
...
kubectl exec -it "$productcatalog_mm_id" -- rm state/mca_fail_calls.txt
