...
class CapacityAnalyzer:
    ...
    def __init__(self, microservice_names, arm_name, microservice_num, microservice_resource_config):
        ...
        self.last_scaling = {}
        # init last scaling with default value
        # to avoid the very first round fail
        for name in self.microservice_names:
            self.last_scaling.update({name: ARMDecision(
                name,
                "",
                1,
                3,
                microservice_resource_config[name]["cpu_request_per_rep"]
                )})
            ...
            if os.path.getsize(self.path) > 0:
                with open(self.path, 'r') as file:
                    ...
                    for arm_decision in lines:
                        ...
                        components = [comp.strip() for comp in arm_decision.split(',')]
                        arm_obj = ARMDecision(
                            ...
                            cpu_request_per_rep=int(components[4])
                        )
    ...
    def _remove_from_state(self, microservice_name):
        ...
        with self.state_lock:
            target_index = -1
            index = 0
            while index < len(self.total_distribute_fail_calls) and target_index == -1:
                decision = self.total_distribute_fail_calls[index]
                if decision.microservice_name == microservice_name:
                    target_index = index
                index += 1
            if target_index == -1:
                print(f"Cannot find {microservice_name} in current state.")
                return False
            self.total_distribute_fail_calls.pop(target_index)
            return True
    ...
    # extract resource data from a list of ms names
    def obtain_part_resource_data(self, name_list):
        resource_data = []
        failed_call = []
        for name in name_list:
            for connection in self._microservice_connections:
                microservice_name =  connection["name"]
                if microservice_name == name:
                    data = self._obtain_resource_data(connection)
                    if data is None:
                        failed_call.append(connection["name"])
                    else:
    ...
    def get_max_reps(self, name):
        for connection in self._microservice_connections:
            if connection["name"] == name:
                stub = connection["stub"]
                request = microservice_manager_pb2.MaxRepRequest()
                try:
                    res = stub.GetMaxReps(request)
                    return res
                except:
                    return None
    ...
    '''
        ...
        Output:
            status_list - List<Dict>:
                ...
                scaling status (str) - (not used here anyways)
            failed_call - List<ARMDecision>
    '''
    
    def distribute_all_instructions(self, arm_decisions):
        # use Reentrant lock because if we do
        # for : add_to_state(), the state of state object 
        # will not be consistent between each loop
        # as the mailman thread might modify it
        with self.state_lock:
            status_list = []
            ...
            for decision in arm_decisions:
                ...
                if status is None:
                    ...
                    # write to disk
                    with self.disk_lock:
                        # create knowledge_base/mca_fail_calls.txt if not exist
                    ...
                    # create mailman after writing to disk
                    # avoid the thread delete from disk before writing to disk
                    threading.Thread(target=self._mailman, args=(decision,)).start()
                else:
                    status_list.append({
    ...
    def _mailman(self, arm_decision):
        ...
        while success is False:
            result = self._send_scaling_instruction(arm_decision)
            if result is None:
        ...
        print(result)
        print(arm_decision)
        print("Live max reps: ", self.get_max_reps(arm_decision.microservice_name))
        print(f"Removed {arm_decision.microservice_name} from state")
        ...
        with self.disk_lock:
            with open(self.path, 'r') as file:
                ...
                while index < len(lines) and target_index == -1:
                    ...
                    index += 1
                if target_index == -1:
                    print(f"Cannot find {arm_decision.microservice_name} in mca_fail_calls")
                else:
...
def run(microservice_names, arm_name, runtime,
    ...
    while (time.time() - start_time) < runtime:
        ...
        # Hold the lock and get the wanted ms names for this round
        # if we extract all then hold lock and filter
        # WRONGGGGG
        # extract all
        # (state list get updated here)
        # hold lock 
        #   get ignore index
        # release lock
        # filter: we use old version of extract all
        with client.state_lock:
            wanted_ms = [name for name in microservice_names if name not in client.total_distribute_fail_calls]
            ignore_ms = [name for name in microservice_names if name not in wanted_ms]
            ignore_data = [decision for decision in client.total_distribute_fail_calls if decision.microservice_name in ignore_ms]
        resource_data, extract_failed_calls = client.obtain_part_resource_data(wanted_ms)

        # update KB for result
        # those being updated by the mailman will be considered
        # as failed to extract to maintain consistency for KB
        client.update_knowledge_base(resource_data, ignore_ms, test_time)
        ...
        print("IGNORE: ", ignore_ms)

        print("Resource data before Smart HPA scaling: ")
        # print newly extracted data
        print("Filter Data: (valid)")
        for data in resource_data:
            data_str = data.microservice_name + ": "
            data_str += str(data.current_reps) + '/' + str(data.max_reps)
            data_str += "\n"
            print(data_str)
        # print incomplete ARMDecision of Smart HPA
        print("Incomplete data: (untouch)")
        for decision in ignore_data:
            data_str = decision.microservice_name + ": "
            data_str += str(decision.feasible_reps) + '/' + str(decision.arm_max_reps)
            data_str += "\n"
            print(data_str)
