...
kubectl logs -f "$result" >> test_three.log &


# run load test
# run locust load test, 2>&1 print error to terminal merge 2 streams into stdout
locust -f ../Load_Script/locustfile.py --host=http://localhost --headless -u 150 -r 0.7 --run-time=500 2>&1 --csv=test_three_locust --csv-full-history
